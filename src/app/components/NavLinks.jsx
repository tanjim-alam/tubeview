    ...
    return (
        <>
            ...
            <li className="relative group w-full lg:hidden">
                <Link href={"#"} onClick={() => showNavLinks(navLink.id)} className='font-medium text-md text-white  w-full block py-2 text-lg px-3' style={{ backgroundColor: `${navLink.color}` }}>{navLink.name} <span className=' absolute right-2 top-3 text-2xl'>{isActive ? <FaAngleUp /> : <FaAngleDown />}</span></Link>
